...
public class cartadapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public cartadapter(Context mContext,List<T> productLists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof EventViewHolder){
            ...
            try {
                ((cartadapter.EventViewHolder) holder).title.setText(((CartResult)dataItem).getBooktitle());
                ((EventViewHolder) holder).author.setText(((CartResult)dataItem).getAuthor());
                int itemCount = Integer.parseInt(((EventViewHolder) holder).valueTextView.getText().toString());
                int price = itemCount * Integer.parseInt(((CartResult)dataItem).getPrice());
                ((EventViewHolder) holder).price.setText(price + "");
                if(((CartResult)dataItem).getDiscount().trim().equals("Donate")){
                    ((EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                    ((EventViewHolder) holder).price.setText("0");
                }else{
                    ((EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                     int finaPrice = (int) Double.parseDouble(((CartResult)dataItem).getDiscountprice()) + 50;
                    ((EventViewHolder) holder).price.setText("" + finaPrice);
                    ((EventViewHolder) holder).discount_text.setText("Rs" + ((CartResult)dataItem).getPrice() + "(" + ((CartResult)dataItem).getDiscount() + ")");
                }
                if(((CartResult)dataItem).getFrontimage()!=null && !((CartResult)dataItem).getFrontimage().equals("")){
                    Picasso.with(mContext)
                            .load(((CartResult)dataItem).getFrontimage())
                            .resize(90,90)
                            .centerInside()
                            .placeholder(R.drawable.placeholder)
                            .error(R.drawable.no_image_available)
                            .into(((cartadapter.EventViewHolder) holder).image);
                }
