 ...
 */
public class WishAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public WishAdapter(Context mContext, List<T> productLists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof EventViewHolder){
            ...
            try {
                ((WishAdapter.EventViewHolder) holder).title.setText(((UserWishListResult)dataItem).getBooktitle());
                ((EventViewHolder) holder).category.setText(((UserWishListResult)dataItem).getAuthor());
                int itemCount = Integer.parseInt(((EventViewHolder) holder).valueTextView.getText().toString());
                int price = itemCount * Integer.parseInt(((UserWishListResult)dataItem).getPrice());

                if(((UserWishListResult)dataItem).getDiscount().trim().equals("Donate")){
                    ((WishAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                    ((EventViewHolder) holder).price.setText("0");
                }else{
                    ((WishAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                    int finaPrice = (int) Double.parseDouble(((UserWishListResult)dataItem).getDiscountprice()) + 50;
                    ((EventViewHolder) holder).price.setText("" + finaPrice);
                    ((WishAdapter.EventViewHolder) holder).discount_text.setText("Rs" + ((UserWishListResult)dataItem).getDiscountprice()+ ((UserWishListResult)dataItem).getPrice() + "(" + ((UserWishListResult)dataItem).getDiscount() + ")");
                }
                if(((UserWishListResult)dataItem).getFrontimage()!=null && !((UserWishListResult)dataItem).getFrontimage().equals("")) {
                    Picasso.with(mContext)
                            .load(((UserWishListResult)dataItem).getFrontimage())
                            .resize(90,90)
                            .centerInside()
                            .placeholder(R.drawable.placeholder)
                            .error(R.drawable.no_image_available)
                            .into(((WishAdapter.EventViewHolder) holder).image);
                }
            }catch (Exception ex){
                ex.printStackTrace();
            }
