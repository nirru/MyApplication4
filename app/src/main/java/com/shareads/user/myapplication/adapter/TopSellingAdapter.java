 ...
 */
public class TopSellingAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public TopSellingAdapter(Context mContext, List<T> productLists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof EventViewHolder){
            ...
            ((TopSellingAdapter.EventViewHolder) holder).title.setText(((TopSellingResult)dataItem).getTitle());
            ((TopSellingAdapter.EventViewHolder) holder).price.setText(((TopSellingResult)dataItem).getPrice());
            if(((TopSellingResult)dataItem).getDiscount().trim().equals("Donate")){
                ((TopSellingAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                ((TopSellingAdapter.EventViewHolder) holder).price.setText("0");
            }else{
                ((TopSellingAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                int finaPrice = (int)Double.parseDouble(((TopSellingResult)dataItem).getDiscountprice()) + 50;
                ((TopSellingAdapter.EventViewHolder) holder).price.setText("" + finaPrice);
                ((TopSellingAdapter.EventViewHolder) holder).discount_text.setText("Rs" + ((TopSellingResult)dataItem).getPrice() + "(" + ((TopSellingResult)dataItem).getDiscount() + ")");
            }
            if (((TopSellingResult)dataItem).getFrontimage()!=null && !((TopSellingResult)dataItem).getFrontimage().equals(""))
                Picasso.with(mContext)
                        .load(((TopSellingResult)dataItem).getFrontimage())
                        .resize(50,50)
                        .centerInside()
                        .placeholder(R.drawable.placeholder)
                        .error(R.drawable.no_image_available)
                        .into(((TopSellingAdapter.EventViewHolder) holder).image);
