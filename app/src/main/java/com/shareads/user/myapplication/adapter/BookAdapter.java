 ...
 */
public class BookAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BookAdapter(Context mContext,List<T> productLists) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ITEM){
            View itemView = LayoutInflater.
                    ...
                    inflate(R.layout.mybooks_row, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof EventViewHolder){
            ...
            try {
                ((BookAdapter.EventViewHolder) holder).title.setText(((MyBookListModel.Result)dataItem).getTitle());
                ((EventViewHolder) holder).author.setText(((MyBookListModel.Result)dataItem).getAuthor());
                if(((MyBookListModel.Result)dataItem).getDiscount().trim().equals("Donate")){
                    ((BookAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                    ((EventViewHolder) holder).price.setText("0");
                }else{
                    ((BookAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                     int finaPrice = (int)Double.parseDouble(((MyBookListModel.Result)dataItem).getDiscountprice()) + 50;
                    ((EventViewHolder) holder).price.setText("" + finaPrice);
                    ((BookAdapter.EventViewHolder) holder).discount_text.setText("Rs" + ((MyBookListModel.Result)dataItem).getPrice() + "(" + ((MyBookListModel.Result)dataItem).getDiscount() + ")");
                }
                if(((MyBookListModel.Result)dataItem).getFrontimage()!=null && !((MyBookListModel.Result)dataItem).getFrontimage().equals("")){
                    Picasso.with(mContext)
                            .load(((MyBookListModel.Result)dataItem).getFrontimage())
                            .resize(90,90)
                            .centerInside()
                            .placeholder(R.drawable.placeholder)
                            .error(R.drawable.no_image_available)
                            .into(((BookAdapter.EventViewHolder) holder).image);
                }
    ...
    public static class EventViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        public EventViewHolder(View itemView) {
            ...
            author = (TextView)itemView.findViewById(R.id.category) ;
