 ...
 */
public class SearchAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SearchAdapter(Context mContext, List<T> productLists) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof EventViewHolder){
            ...
            try {
                ((SearchAdapter.EventViewHolder) holder).title.setText(((SearchResult)dataItem).getTitle());
                ((SearchAdapter.EventViewHolder) holder).category.setText(((SearchResult)dataItem).getAuthor());
                int itemCount = Integer.parseInt(((EventViewHolder) holder).valueTextView.getText().toString());
                int price = itemCount * Integer.parseInt(((SearchResult)dataItem).getPrice());

                if(((SearchResult)dataItem).getDiscount().trim().equals("Donate")){
                    ((SearchAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                    ((SearchAdapter.EventViewHolder) holder).price.setText("0");
                }else{
                    ((SearchAdapter.EventViewHolder) holder).relative_Discount.setVisibility(View.GONE);
                     Double finaPrice = Double.parseDouble(((SearchResult)dataItem).getDiscountprice()) + 50;
                    ((SearchAdapter.EventViewHolder) holder).price.setText("" + finaPrice);
                    ((SearchAdapter.EventViewHolder) holder).discount_text.setText("Rs" + ((SearchResult)dataItem).getPrice() + "(" + ((SearchResult)dataItem).getDiscount() + ")");

                }
                if (((SearchResult)dataItem).getFrontimage()!=null && !((SearchResult)dataItem).getFrontimage().equals(""))
                Picasso.with(mContext)
                        .load(((SearchResult)dataItem).getFrontimage())
                        .resize(90,90)
                        .centerInside()
                        .placeholder(R.drawable.placeholder)
                        .error(R.drawable.no_image_available)
                        .into(((SearchAdapter.EventViewHolder) holder).image);
            }catch (Exception ex){
                ex.printStackTrace();
            }
